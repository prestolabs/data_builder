...
class read_product_info:
    ...
    def get_symbol_name(self,date):
        tw_symbol_list = []
        twse_symbol_list = self.get_twse_symbol(date)
        tpex_symbol_list = self.get_tpex_symbol(date)
        tw_symbol_list.extend(twse_symbol_list)
        tw_symbol_list.extend(tpex_symbol_list)
        return sorted(tw_symbol_list)

    def get_twse_symbol(self,date):
        path = twse_interval_path.format(date=date)
        try:
            with h5py.File(path, "r") as h5file:
                print(f"Successfully opened file: {path}")
    ...
    def get_tpex_symbol(self,date):
        path = tpex_interval_path.format(date=date)
        try:
            with h5py.File(path, "r") as h5file:
                print(f"Successfully opened file: {path}")
                data = h5file["universe"][:]  
